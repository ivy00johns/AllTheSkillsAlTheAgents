...
[Quick Start](#quick-start) · [Why this exists](#why-this-exists) · [Architecture](#architecture) · [Skill catalog](#skill-catalog) · [Other hosts](#also-works-on-ten-other-hosts) · [Roadmap](#roadmap)
...
**📚 Full skill table** (click to expand)
...
**📖 Full script reference** (click to expand)
...
**"**`/sync-skills` **says skills already exist"**
...
**"Linter fails on macOS but passes on Linux (or vice versa)"**
...
**"My non-Claude-Code host doesn't see all 39 skills"**
...
**"Stderr is full of** `stripped allowed_tools/owns` **lines"**
...
**"Orchestrator complains about file-ownership conflicts"**
...
**"**`install.sh` **can't find my host's install directory"**
...
- **Skill library** — 39 skills, six categories, all linted
- **Multi-tool installer** — convert / install / lint, eleven host adapters
- **CI matrix** — Ubuntu + macOS lint on every push
- **Contract-first specs** — OpenAPI / AsyncAPI / Pydantic / TypeScript / JSON Schema templates
- **QA gate** — `qa-report.json` schema with critical / high / medium / low blockers
- **Two-runtime degradation** — Agent Teams → subagents → sequential, host-detected
...
🧰 Built by humans + the agents they coordinate.
